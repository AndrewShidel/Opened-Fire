...
function sendNewZombies(nx, ny, _zt, speed) {
    var json = {
        type: "NewZombies",
        nx: nx,
        ny: ny,
...
function NewZombies(json) {
    ...
    zy.push(json.ny);
    zt.push(json.zt);
    zspeed.push(json.speed);

    numZombies = zx.length;
...
function game(up, down, left, right) {
    ...
    document.getElementById("b").onmouseup = function (e) {
        up2(e);
    };

    document.getElementById("b").ontouchstart = function (e) {
        e.preventDefault();
        tdown(e);
    };
    document.getElementById("b").ontouchmove = function (e) {
        e.preventDefault();
        tmove(e);
    };
    document.getElementById("b").ontouchend = function (e) {
        e.preventDefault();
        tup(e);
    };
...
function update() {
    ...
    if (Math.random() < difficulty) {
        ...
        if (rand <= .4) {
            ...
            newZombie(Math.round(Math.random()) * w, Math.random() * h);
        }
    }
...
function updateForPlayer() {
    for (var i = 0; i < zx.length; i++) {
        var tempx, tempy;
        if (dst(zx[i], zy[i], x, y) < dst(zx[i], zy[i], user2x, user2y)) {
            tempx = x;
            tempy = y;
        } else {
            tempx = user2x;
...
function postdraw() {
    ...
    ctx1.drawImage(playerImg2, ufx2, ufy2, frameWidth, frameHeight, user2x, user2y, frameWidth, frameHeight);
...
function newZombie(x, y) {
    ...
    numZombies++;
    sendNewZombies(x, y, zt[numZombies - 1], zspeed[numZombies-1]);
}
